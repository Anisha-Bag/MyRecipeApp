...
fun CategoryDetailScreen(category: Category){
        ...
        horizontalAlignment = Alignment.CenterHorizontally)
    {
        ...
        Image(painter = rememberAsyncImagePainter(model = category.strCategoryThumb),
            ...
            modifier = Modifier
                .wrapContentSize()//it will take as much space as it requires
                .aspectRatio(1f)//means it will be as wide and high it could be
        )

        //How can we scroll the text
        Text(text=category.strCategoryDescription,
            textAlign = TextAlign.Justify,
            modifier= Modifier.verticalScroll(rememberScrollState())//Allows us to scroll through the text
        )
